...
function selecciona()
{
	//m=parent.parent.document.getElementById("idGenMes").value;
	parent.parent.document.getElementById("descTipoTipificacion").value=$('#siglasGS').val();
//	parent.parent.document.getElementById("senpladesDescripcion").value=descSP;
	parent.parent.document.getElementById("idGenTipoTipificacion").value=$('#paisCdg').val();
	parent.parent.selectTipo('<?php echo $_GET['m']?>','<?php echo $_GET['a']?>');
    ...
    <tr>
        <td width="100%">
            ...
            <ul id="tpais" class="easyui-tree" animate="true" style="font-size:10px"
                url="treeViewTipificacionSU.php?id=<?php echo (isset($_GET['id'])?$_GET['id']:0) ?><?php echo isset($_GET['pais'])?'&pais='.$_GET['pais']:'' ?><?php echo isset($_GET['parroquia'])?'&parroquia='.$_GET['parroquia']:'' ?>">
            </ul>
          </div>
        </td>
    </tr>
