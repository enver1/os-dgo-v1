...
$rowt           = array();
$idGenTipoOperativo   = strip_tags($encriptar->getDesencriptar($_GET['c'], $_SESSION['usuarioAuditar']));
$directorioArbol  = '../' . $directorio . '/includes/treeViewTipoOperativo.php';
if ($idGenTipoOperativo > 0) {
    $rowt = $TipoOperativoOrden->getEditTipoOperativoOrden($idGenTipoOperativo);
}
$FormTipoOperativoOrden->getForma($rowt, $TipoOperativoOrden->getIdCampo(), $opc, $directorioArbol, 'Accion/Detalle/Descripción', '', '220');
?>
<script type="text/javascript">
    ...
    function getregistroN(c) {
        ...
        $('#genTipoOperativo_idGenTipoOperativo').val($('#idGenTipoOperativo').val());
        $('#descripcionPadre').val($('#descripcion').val());
        $('#idGenTipoOperativo').val('');
        $('#descripcion').val('');
                    ...
                    if (isset($_SESSION['privilegios'])) {
                        switch (substr($_SESSION['privilegios'], 0, 2)) {
                            case '11': //SI Insert y SI Update
                                echo '<input type="button" name="enviar" onclick="grabaregistro(1,\'id' . $TipoOperativoOrden->getIdCampo() . '\')" value="Grabar"  class="boton_save" />';
                                break;
                            case '10'; //SI Insert y NO Update
                                echo '<input type="button" name="enviar" onclick="grabaregistro(2,\'id' . $TipoOperativoOrden->getIdCampo() . '\')" value="Grabar"  class="boton_save" />';
                                break;
                            case '01'; // NO Insert y SI Update
                                echo '<input type="button" name="enviar" onclick="grabaregistro(3,\'id' . $TipoOperativoOrden->getIdCampo() . '\')" value="Grabar"  class="boton_save" />';
                                break;
