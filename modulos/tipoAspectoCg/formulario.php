...
$rowt           = array();
$idCgTipoAspecto   = strip_tags($encriptar->getDesencriptar($_GET['c'], $_SESSION['usuarioAuditar']));
$directorioArbol  = '../' . $directorio . '/includes/treeViewTipoAspectoCg.php';
if ($idCgTipoAspecto > 0) {
    $rowt = $tipoAspectoCg->getEditTipoAspectoCg($idCgTipoAspecto);
}
$FormTipoAspectoCg->getForma($rowt, $tipoAspectoCg->getIdCampo(), $opc, $directorioArbol, 'Accion/Detalle/Descripción', '', '220');
?>
<script type="text/javascript">
    ...
    function getregistroN(c) {
        ...
        $('#cg_idCgTipoAspecto').val($('#idCgTipoAspecto').val());
                    ...
                    if (isset($_SESSION['privilegios'])) {
                        switch (substr($_SESSION['privilegios'], 0, 2)) {
                            case '11': //SI Insert y SI Update
                                echo '<input type="button" name="enviar" onclick="grabaregistro(1,\'id' . $tipoAspectoCg->getIdCampo() . '\')" value="Grabar"  class="boton_save" />';
                                break;
                            case '10'; //SI Insert y NO Update
                                echo '<input type="button" name="enviar" onclick="grabaregistro(2,\'id' . $tipoAspectoCg->getIdCampo() . '\')" value="Grabar"  class="boton_save" />';
                                break;
                            case '01'; // NO Insert y SI Update
                                echo '<input type="button" name="enviar" onclick="grabaregistro(3,\'id' . $tipoAspectoCg->getIdCampo() . '\')" value="Grabar"  class="boton_save" />';
                                break;
