...
$tStructure = array(
	'idDgoResAct' => 'idDgoResAct',
	'idDgoCargo' => 'idDgoCargo',
	'idDgoVisita' => 'idDgoVisita',
	'idDgoActividad' => 'idDgoActividad',
);
...
if ($_POST[$idcampo] == '' or $_POST[$idcampo] == 0) {
	...
	if (!empty($descripcion)) {
		...
		$sql = "select * from " . $sNtabla . " where (" . $whered . ") and idDgoVisita='" . $_POST['idDgoVisita'] . "' and idDgoActividad='" . $_POST['idDgoActividad'] . "'";
		$rs = $conn->query($sql);
...
/*===== Caso contrario si el ID es diferente de 0 o de espacio esta editando el registro y hace un UPDATE ======*/ {
	...
	if (!empty($descripcion)) {
		...
		$sql = "select * from " . $sNtabla . " where (" . $whered . ") and idDgoVisita='" . $_POST['idDgoVisita'] . "' and idDgoActividad='" . $_POST['idDgoActividad'] . "' and " . $idcampo . "!=" . $_POST[$idcampo];
		$rs = $conn->query($sql);
		if ($row = $rs->fetch(PDO::FETCH_ASSOC)) {
			redirect('../../../../funciones/error.php?errno=1');
			$noduplicado = false;
