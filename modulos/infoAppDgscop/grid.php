...
$gridS               = $formInfoAppDgscop->getGrillaInfoAppDgscop(); //Campos de la Grilla
$columnas            = count($gridS) + 2;
$idcampo             = $InfoAppDgscop->getIdCampo();
$data                = $InfoAppDgscop->getInfoAppDgscop('APP_DGSCOP');
...
if (!empty($data)) {
    ...
    foreach ($data as $row) {
        ...
        if (isset($_SESSION['privilegios']) && substr($_SESSION['privilegios'], 1, 1) == 1) {
            if ($simple != 1) {
                $path = FuncionesGenerales::getPathAlmacenamiento('descargas/operaciones/appDgscop/imagenes/') . $row['icono'];
                $foto = explode('.', $row['icono']);
