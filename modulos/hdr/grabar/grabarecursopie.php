...
if($_POST['id'.$idcampo]=='' or $_POST['id'.$idcampo]==0)
	...
	/*------------------------------*/
	// *** CAMBIAR ***
	$sentencia = $conn->prepare("insert into ".$sNtabla." (idHdrRuta,idHdrEstadoRecurso,nominativo, telefonoRecurso, radioRecurso, obsHdrRecurso, usuario, ip) values((select  idHdrRuta from hdrRuta where sha1(idHdrRuta)=?),?,?,?,?,?,?,?)");
	...
	if ($row=$rs->fetch(PDO::FETCH_ASSOC))
		...
		$sentencia = $conn->prepare("update hdrRecurso set idHdrEstadoRecurso=?,nominativo=?, telefonoRecurso=?, radioRecurso=?, obsHdrRecurso=?, usuario=?, ip=? where idHdrRecurso=?");
		$sentencia->bindParam(1, $_POST['idHdrEstadoRecurso']);
		$sentencia->bindParam(2, $_POST['nominativo']);
		$sentencia->bindParam(3, $_POST['telefonoRecurso']);
		$sentencia->bindParam(4, $_POST['radioRecurso']);
		$sentencia->bindParam(5, $_POST['observacion']);
		$sentencia->bindParam(6, $_SESSION['usuarioAuditar']);
		$sentencia->bindParam(7, realIP());
		$sentencia->bindParam(8, $_POST['idHdrRecurso']);
		/*-----------------------------------*/
