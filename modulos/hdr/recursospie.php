...
*/
$sql = "SELECT a.idHdrRecurso, a.nominativo, b.descripcion estado FROM hdrRecurso a, hdrEstadoRecurso b WHERE a.idHdrEstadoRecurso = b.idHdrEstadoRecurso AND sha1(a.idHdrRuta)='".$_REQUEST['recno']."' and ISNULL(a.idHdrVehiculo) ORDER BY a.idHdrRecurso";
...
//loop por cada registro
	while ($rowRE = $rs->fetch(PDO::FETCH_ASSOC)){
		...
		<a href="/operaciones/modulos/hdr/recursospieforma.php?id='.sha1($rowRE['idHdrRecurso']).'&recno='.$_GET['recno'].'&pesta='.$_GET['pesta'].'&opc='.$_GET['opc'].'" onclick="return GB_showPage(\'R E C U R S O S\',this.href)"><img src="/operaciones/imagenes/Nominativo.png"/>
		</a>';
		...
		echo '
		<a class="tooltipIM" href="/operaciones/modulos/hdr/integrantesforma.php?id='.sha1($rowRE['idHdrRecurso']).'&recno='.$_GET['recno'].'&pesta='.$_GET['pesta'].'&opc='.$_GET['opc'].'" onclick="return GB_showPage(\'I N T E G R A N T E S\',this.href)"><img src="/operaciones/imagenes/Policia.png"/>
		...
		<a class="tooltipIM" href="/operaciones/modulos/hdr/sectoresforma.php?id='.sha1($rowRE['idHdrRecurso']).'&recno='.$_GET['recno'].'&pesta='.$_GET['pesta'].'&opc='.$_GET['opc'].'" onclick="return GB_showPage(\'S E C T O R E S\',this.href)"><img src="/operaciones/imagenes/Sector.png"/>';
...
echo "</div>";
?>
<a href="/operaciones/modulos/hdr/recursospieforma.php?id=0<?php echo '&recno='.$_GET['recno']?>&pesta=<?php echo $_GET['pesta'] ?>&opc=<?php echo $_GET['opc'] ?>" class="button" onclick="return GB_showPage('R E C U R S O S', this.href)"><span>Nuevo</span></a>
<br>
<?php
