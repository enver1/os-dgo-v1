...
$idcampo             = $RegistroNovElec->getIdCampoRegistroNovElec(); //Primary Key Table
$gridS               = $FormRegistroNovElec->getGrillaRegistroNovElec(); //Campos de la Grilla
$columnas            = count($gridS) + 2;

$data = $RegistroNovElec->getRegistroNovElec();
    ...
    foreach ($data as $row) {
        ...
        if (isset($_SESSION['privilegios']) && substr($_SESSION['privilegios'], 1, 1) == 1) {
            if ($simple != 1) {
                ...
                if (!empty($row['imagen']) && isset($row['imagen'])) {
                    $foto = explode('.', $row['imagen']);
                    if (!empty($foto[1])) {
                        $path = FuncionesGenerales::getPathAlmacenamiento('descargas/movil/operaciones/elecciones2021/') . $row['imagen'];
                        ...
                        echo '<td style="text-align:center"><a href="' . $imagen . '" onclick="return GB_showPage(\'Imagen Novedades\', this.href)" target="_blank"class="btn btn-danger pull-right"><span class="glyphicon glyphicon-file"</span><img src="../imagenes/btnVer.png" width="20" height="20"></a></td>';
                    } else {
