...
$gridS = $formCrearOrdenServicio->getGrillaCrearOrdenServicio();
$datosUsuario =    $ambitoGestionOrden->getDatosUsuariosSenplades($_SESSION['usuarioAuditar']);
//$data      = $encriptar->getEncriptar($crearOrdenServicio->getSqlCrearOrdenServicio($datosUsuario['idGenGeoSenplades']), $_SESSION['usuarioAuditar']);
$data = $crearOrdenServicio->getCrearOrdenServicio($datosUsuario['idGenGeoSenplades']);
$idcampo = $crearOrdenServicio->getIdCampo();
$simple = isset($simple) ? $simple : 0;
?>
<hr><br>
<div>
    <span style="background-color:#fcc0c8;border:solid 1px #444;padding:2px 5px;font-size:9px">&nbsp;
    </span>Ordenes en Estado Temporal
    &nbsp;&nbsp;
    <span style="background-color:#ffffff;border:solid 1px #444;padding:2px 5px;font-size:9px">&nbsp;
    </span> Ordenes Generadas
    &nbsp;&nbsp;
</div>
<br>
<?php
