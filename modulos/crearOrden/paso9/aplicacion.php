...
if (isset($_SESSION['usuarioAuditar'])) {
	...
	$usuarioactual = 0;
	$idDgoOrdenServicio = isset($_GET['id']) ? $_GET['id'] : 0;
} else {
...
<script>
	$(function() {
		...
		var urlt = 'modulos/crearOrden/paso9/botones.php?id=<?php echo $idDgoOrdenServicio ?>&a=1';
		$('#formulario').load(urlt);
		$('#resultado').load('modulos/crearOrden/paso9/buscaEstado.php?id=<?php echo $idDgoOrdenServicio ?>');
	});
	...
	function comprobarEstado() {
		...
		$.ajax({
			type: 'GET',
			url: "modulos/crearOrden/paso9/activaOrdenServicio.php",
			data: {
				idDgoOrdenServicio: cas,
			},
	...
	function verifica(idDgoOrdenServicio) {
		var c = idDgoOrdenServicio;
		if (c == '') {
			Swal.fire(
				'No Existe Datos',
			...
			$.post("modulos/crearOrden/paso9/verifica.php", {
				id: c
			}, function(data) {
				$("#resultado").html(data);
				var urlt = 'modulos/crearOrden/paso9/botones.php?id=' + c + '&a=' + c;
				$('#formulario').load(urlt);
			});
		}
	}
	...
	function previsualiza() {
		var idDgoOrdenServicio = "<?php echo $_GET['id'] ?>";
		var url = "modulos/crearOrden/paso9/imprimeRepo.php?id=" + idDgoOrdenServicio;
		var l = screen.width;
		var t = screen.height;
	...
	function procesar() {
		...
		Swal.fire({
			...
			if (result.isConfirmed) {
				...
				var urlt = 'modulos/crearOrden/paso9/procesa.php';
				$.ajax({
					type: "POST",
					url: urlt,
					data: {
	...
	function imprimir() {
		var idDgoOrdenServicio = "<?php echo $_GET['id'] ?>";
		var url = "modulos/crearOrden/paso9/imprime.php?id=" + idDgoOrdenServicio;
		var l = screen.width;
		var t = screen.height;
		var opts = 'scrollbars=yes,toolbar=no,width=' + screen.width + ',height=' + screen.height + ',top=' + t + ' ,left=' + l;
		var name = 'pdf';
