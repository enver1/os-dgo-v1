...
class TipoServicioOp extends Transaccion
{
    ...
    {
        $sql = "SELECT a.idHdrTipoServicio,a.descripcion  from hdrTipoServicio a ORDER BY a.idHdrTipoServicio DESC";
        return $sql;
    }
    ...
    public function eliminarTipoServicioOp($idHdrTipoServicio)
    {
        if (!empty($idHdrTipoServicio)) {
            $respuesta = $this->borrar($this->tabla, $idHdrTipoServicio);
        } else {
